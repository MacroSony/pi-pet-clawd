...
function createIngressRequestHandler(options = {}, counters = { acceptedCount: 0, rejectedCount: 0 }) {
  ...
  return (req, res) => {
    ...
    const isPetInboxClaim = req.method === "POST" && req.url === "/pet-inbox/claim";
    const isPetInboxSettle = req.method === "POST" && req.url === "/pet-inbox/settle";
    const isPetPeerEndpoint = req.method === "POST" && (
      req.url === "/pet-peer/catalog"
      || req.url === "/pet-peer/send"
      || req.url === "/pet-peer/claim"
    ...
    const isPetChatEndpoint = req.method === "POST" && req.url === "/pet-chat/complete";
    const nonce = (isPetExpression || isPetInboxClaim || isPetInboxSettle || isPetPeerEndpoint || isPetTeamEndpoint || isPetChatEndpoint)
      ? headerValue
